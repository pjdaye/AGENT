...
class ConcreteStateFeaturize:
    ...
    @staticmethod
    def convert_to_feature_frame(context, concrete_state, measure_color_distance=True):
        data = []

        parent_map = {}
        sibling_map = {}
        input_widgets = []
        best_widget_distance = 9999
        ...
        for widget in concrete_state['widgets'].values():
            ...
            try:
                x_percent = int(widget['properties']['xPercent'])
            except:
                x_percent = 0

            try:
                y_percent = int(widget['properties']['yPercent'])
            except:
                y_percent = 0

            try:
                font_size = int(widget['properties']['fontSize'])
            except:
                font_size = 0

            try:
                font_weight = int(widget['properties']['font-weight'])
            except:
                font_weight = 0
        ...
        #     distance_from_input_widget = data_row[-3]
        #     if distance_from_input_widget == 9999:
        #         data_row[-3] = best_widget_distance

        # for data_row in data:
        #     distance_from_input_widget = data_row[-2]
        #     distance_from_input_widget /= total_distance
        #     data_row[-2] = distance_from_input_widget
